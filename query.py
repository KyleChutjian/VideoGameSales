...
def query():
    ...
    ''', cnx)
    fig = px.bar(df, title='Sales By Game', x='name', y=['na_sales', 'eu_sales', 'jp_sales', 'other_sales'])
    fig.show()
    ...
    platform_barchart_df = pd.read_sql('''
    SELECT p.platform_name as 'Platform', SUM(g.na_sales) as 'NA Sales', SUM(g.eu_sales) as 'EU Sales', SUM(g.jp_sales) as 'JP Sales', SUM(g.other_sales) as 'Other Sales'
    FROM Game g, Platform p
    WHERE g.genre_id = p.platform_id
    GROUP BY p.platform_id
    ORDER BY SUM(g.global_sales) DESC;
    ''', cnx)
    platform_barchart_fig = px.bar(platform_barchart_df, title='Sales by Platform', x='Platform', y=['NA Sales', 'EU Sales', 'JP Sales', 'Other Sales'])
    platform_barchart_fig.show()
